...
describe("EShop", () => {
  ...
  it("should show error message if data fetching failed", async () => {
    ...
    expect(
      ...
      await screen.findByText(
        "Axios Error with Message: Request failed with status code 500"
      )
    ).toBeInTheDocument();
  });
  ...
  it("Should filter for male cats", async () => {
    ...
    expect(cards.length).toBe(6);
    const selectMaleCats: HTMLSelectElement =
      screen.getByLabelText("filterByGender");
  ...
  it("Should filter favorite only cats", async () => {
    render(<Eshop />);
    const cards = await screen.findAllByRole("article");
    const selectFavCats: HTMLSelectElement =
      screen.getByLabelText("filterByFavorite");
  ...
  it("Should update favorite cats if clicked on any unfavorite heart button", async () => {
    render(<Eshop />);
    const cards = await screen.findAllByRole("article");
